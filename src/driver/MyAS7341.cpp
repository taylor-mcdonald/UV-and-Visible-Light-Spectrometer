...
bool Adafruit_AS7341::setMode(as7341_mode mode) {
  Adafruit_BusIO_Register mode_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CONFIG);
  Adafruit_BusIO_RegisterBits mode_en =
      Adafruit_BusIO_RegisterBits(&mode_reg, 2, 0);
  return mode_en.write(mode);
}
...
/**
 * @brief Enables spectral data interrupts
 *  Bit 3 of INTENAB Register (Addr: 0xF9  Bank: 0)
 * @param enable_interupt true: enabled false: disabled
 * @return true: success false: failure
 */
bool Adafruit_AS7341::enableSpectralInterupt(bool enable_interupt) {
  ...
  return spec_enable_bit.write(enable_interupt);
}

/**
 * @brief Enables automatic control of spectral gain (Channels F1 - F8)
 * Bit 2 of CFG8 Register (Addr: 0xB1  Bank: 0)
 * @param enable_measurement true: enabled false: disabled
 * @return true: success false: failure
 */
bool Adafruit_AS7341::enableSpectralAutoGainControl(bool enable_int) {

  Adafruit_BusIO_Register enable_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CFG8);
  Adafruit_BusIO_RegisterBits spec_AGC_enable_bit =
      Adafruit_BusIO_RegisterBits(&enable_reg, 1, 2);
  return spec_AGC_enable_bit.write(enable_int);
}

/**
 * @brief Enables the wait time function between measurements.  Set WTIME in
 * WTIME Register (Addr: 0x83  Bank: 0) first.
 * Bit 3 of ENABLE Register (Addr: 0x80  Bank: 0)
 * @param enable_measurement true: enabled false: disabled
 * @return true: success false: failure
 */
bool Adafruit_AS7341::enableWait(bool enable_int) {

  Adafruit_BusIO_Register enable_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_ENABLE);
  Adafruit_BusIO_RegisterBits WEN_enable_bit =
      Adafruit_BusIO_RegisterBits(&enable_reg, 1, 3);
  return WEN_enable_bit.write(enable_int);
}

/**
 * @brief Enables automatic control of Flicker gain
 * Bit 3 of CFG8 Register (Addr: 0xB1  Bank: 0)
 * @param enable_measurement true: enabled false: disabled
 * @return true: success false: failure
 */
bool Adafruit_AS7341::enableFlickerAutoGainControl(bool enable_int) {
  
  Adafruit_BusIO_Register enable_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CFG8);
  Adafruit_BusIO_RegisterBits FD_AGC_enable_bit =
      Adafruit_BusIO_RegisterBits(&enable_reg, 1, 3);
  return FD_AGC_enable_bit.write(enable_int);
}

/**
 * @brief Enable the Sync signal to be applied on output pin INT
 *  Bit 2 of CONFIG Register (Addr: 0x70  Bank: 0)
 * @param enable_int_sel true: enabled false: disabled
 * @return true: success false: failure
 */
bool Adafruit_AS7341::enableINTsel(bool enable_int_sel) {

  Adafruit_BusIO_Register config_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CONFIG);

  Adafruit_BusIO_RegisterBits int_sel_enable_bit =
      Adafruit_BusIO_RegisterBits(&config_reg, 1, 2);
  return int_sel_enable_bit.write(enable_int_sel);
}
...
bool Adafruit_AS7341::enableSMUX_non_block(void) {
...
/**
 * @brief Get register CFG8 and check the bits
 *
 *  This is for troubleshooting / valiadtion
 */
uint8_t Adafruit_AS7341::getCFG8(void) {
  Adafruit_BusIO_Register cfg8_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CFG8, 1);
  return cfg8_reg.read();
}

uint8_t Adafruit_AS7341::getRegister(uint8_t reg) {
  Adafruit_BusIO_Register read_reg =
      Adafruit_BusIO_Register(i2c_dev, reg, 1);
  return read_reg.read();
}

bool Adafruit_AS7341::getSP_AGC(void) {
  Adafruit_BusIO_Register cfg8_reg =
      Adafruit_BusIO_Register(i2c_dev, AS7341_CFG8, 1);

  Adafruit_BusIO_RegisterBits sp_agc_bit =
      Adafruit_BusIO_RegisterBits(&cfg8_reg, 1, 2);  // 1 bit at pos 2
...
uint8_t Adafruit_AS7341::readFIFO(uint16_t *buffer, uint8_t maxSamples) {
    ...
    uint8_t count = min((int)level, (int)maxSamples);
